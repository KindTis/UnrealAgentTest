...
1차 마일스톤에서는 **실행 중인 게임/에디터 attach 기반**으로 자연어 테스트 지시를 시나리오 JSON으로 변환하고, 상태 기반 이동/공격/처치 판정 및 종료 정책 옵션까지 포함한 E2E 자동화를 운영 가능한 기준선으로 확정한다.
...
- 자연어 지시(예: "A무기로 정면 몬스터 처치")를 테스트 시나리오 JSON으로 변환하는 시나리오 작성 계층 구현.
...
- 자연어 요청을 시나리오 JSON으로 변환하기 위한 시나리오 포맷 스키마.
...
11. `IA-04`/`IA-05`가 attach 실행 모드 기준 자연어 입력을 시나리오 JSON으로 변환하는 인터페이스를 구현한다.
...
- 통합 검증:
  ...
  - attach 모드에서 자연어 입력을 시나리오 JSON으로 변환 후 동일 시나리오 재실행 시 결과 일관성 검증.
